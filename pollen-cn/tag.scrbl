...
以这种方式输入属性可能很麻烦。因此，为方便起见，新的标签函数提供了一种替代方法：任何关键字参数及其值都将被解释为属性。
...
请注意，虽然默认标签函数通常用于生成标记的 X 表达式，但它们不会对输入实施任何限制，因此它们也不保证您实际上会获得有效的标记 X 表达式作为输出。这是故意的——默认标签函数是为了方便编码，它们的输出很可能会被其他标签函数处理，所以在这里提出错误还为时过早。
